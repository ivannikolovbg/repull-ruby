...
module Repull
  ...
  class ReservationFinancials < ApiModelBase
    # GUEST-side stay total in `currency` — what the guest paid, not the host payout. Number, not string. For the host payout see `financials.host.revenue`.
    ...
    # Channel cancellation policy code, verbatim from the reservation. Airbnb codes look like `strict_14_with_grace_period`, `moderate`, `flexible`, `tiered_pricing_non_refundable`; Booking.com reports a numeric policy id. Omitted when the channel did not supply one.
    ...
    def self.openapi_types
      {
        ...
        :'host' => :'ReservationHostFinancials',
        :'guest' => :'ReservationGuestFinancials'
      }
    end
    ...
    def hash
      [total_price, currency, payment_status, cancellation_policy, host, guest].hash
    end
