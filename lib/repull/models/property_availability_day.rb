...
module Repull
  ...
  class PropertyAvailabilityDay < ApiModelBase
    ...
    # Whether the property is bookable on this date. `false` when the calendar marks the date booked or blocked. Only dates we actually hold a calendar row for appear in `days`, so this is never a guess — a date with no data is listed in `coverage.missingDates` instead.
    attr_accessor :available

    # Nightly price for this date in the property currency. Falls back to the property's default nightly price when the calendar row itself carries no price.
    attr_accessor :price

    # Minimum-stay requirement for a stay starting on this date. Falls back to the listing-level default min-nights when the calendar row carries none.
