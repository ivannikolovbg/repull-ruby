...
module Repull
  ...
  class AirbnbListing < ApiModelBase
    # Airbnb listing ID
    attr_accessor :id
    ...
    # Listing status (active, unlisted, etc.)
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::AirbnbListing` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::AirbnbListing`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, name, status, property_type, room_type, bedrooms, bathrooms, max_guests, thumbnail_url].hash
    end
