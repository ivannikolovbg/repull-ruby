...
module Repull
  ...
  class AlterationWebhookObject < ApiModelBase
    # Repull-internal alteration id (`reservation_alterations.id`).
    attr_accessor :id

    # Provider-side alteration/resolution id.
    attr_accessor :alteration_id

    # Source channel. Currently always `airbnb`.
    attr_accessor :channel

    # Repull reservation id the alteration targets. Pass to `GET /v1/reservations/{id}`.
    attr_accessor :reservation_id

    # Workspace (customer) id.
    attr_accessor :customer_id

    # Alteration lifecycle status.
    attr_accessor :status

    # Who requested the alteration.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::AlterationWebhookObject` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::AlterationWebhookObject`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, alteration_id, channel, reservation_id, customer_id, status, initiator].hash
    end
