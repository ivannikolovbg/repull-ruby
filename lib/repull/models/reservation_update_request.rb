...
module Repull
  ...
  class ReservationUpdateRequest < ApiModelBase
    ...
    # Move the reservation to another property in this workspace. Combined with dates, it is applied as ONE move so the access code is re-issued once.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::ReservationUpdateRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::ReservationUpdateRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [check_in, check_out, check_in_time, check_out_time, guest_count, listing_id].hash
    end
