...
module Repull
  ...
  class ListingAmenity < ApiModelBase
    # Canonical amenity key (e.g. `wifi`, `pool`, `parking`).
    attr_accessor :amenity_key

    # Optional grouping (e.g. `essentials`, `safety`).
    attr_accessor :category

    # `true` when the listing has this amenity, `false` when it has been explicitly opted out.
    attr_accessor :is_present

    # Optional free-form instruction for the guest (e.g. WiFi password, parking notes).
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::ListingAmenity` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::ListingAmenity`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
