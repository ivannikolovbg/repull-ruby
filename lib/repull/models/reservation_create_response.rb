...
module Repull
  class ReservationCreateResponse < ApiModelBase
    ...
    # The price the pricing engine derived for the stay. Reservations created through this endpoint are NOT priced from the request — see the operation description.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::ReservationCreateResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::ReservationCreateResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, confirmation_code, listing_id, platform, status, check_in, check_out, guest_id, total_price, currency].hash
    end
