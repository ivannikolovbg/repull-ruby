...
module Repull
  class AvailabilityBatchWriteRequest < ApiModelBase
    ...
    # ISO dates. Capped at 731 — Airbnb refuses calendar writes spanning more.
    attr_accessor :dates
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::AvailabilityBatchWriteRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::AvailabilityBatchWriteRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @dates.length > 731
        invalid_properties.push('invalid value for "dates", number of items must be less than or equal to 731.')
      end
      ...
      if @property_ids.length > 500
        invalid_properties.push('invalid value for "property_ids", number of items must be less than or equal to 500.')
      end
    ...
    def valid?
      ...
      return false if @dates.length > 731
      return false if @property_ids.nil?
      return false if @property_ids.length > 500
      true
    end
    ...
    def max_nights=(max_nights)
      if max_nights.nil?
        fail ArgumentError, 'max_nights cannot be nil'
      end
    ...
    def dates=(dates)
      ...
      if dates.length > 731
        fail ArgumentError, 'invalid value for "dates", number of items must be less than or equal to 731.'
      end
    ...
    def property_ids=(property_ids)
      ...
      if property_ids.length > 500
        fail ArgumentError, 'invalid value for "property_ids", number of items must be less than or equal to 500.'
      end
    ...
    def hash
      [available, price, min_nights, max_nights, dates, property_ids].hash
    end
