...
module Repull
  ...
  class ListingPricingHistoryEntry < ApiModelBase
    ...
    # Price actually written to the calendar. `null` when status is `pending` or `declined`. For now, when `status=applied` this equals `recommendedRate` because the apply path writes the recommendation verbatim.
