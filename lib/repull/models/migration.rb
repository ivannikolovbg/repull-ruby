...
module Repull
  ...
  class Migration < ApiModelBase
    # Pass as `X-Workspace-Id` to read this property manager's listings, reservations and conversations through the regular endpoints.
    attr_accessor :workspace_id

    attr_accessor :name

    # Your own id for this property manager, as sent when the migration was created.
    attr_accessor :external_ref

    attr_accessor :created_at

    # `awaiting_connection` — not connected yet. `importing` — the first import is running. `imported` — data is in and kept fresh until cutover. `failed` — the last import failed (see `connections[].import.error`). `cut_over` — the source was disconnected. `deactivated` — the migration was deleted.
    attr_accessor :state

    attr_accessor :cutover_at
    ...
    def self.openapi_types
      {
        ...
        :'state' => :'String',
        :'cutover_at' => :'Time',
        :'connections' => :'Array<MigrationConnectionsInner>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::Migration` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::Migration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cutover_at')
        self.cutover_at = attributes[:'cutover_at']
      end
    ...
    def hash
      [workspace_id, name, external_ref, created_at, state, cutover_at, connections, counts].hash
    end
