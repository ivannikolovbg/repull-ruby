...
module Repull
  ...
  class StudioDeployment < ApiModelBase
    attr_accessor :deployment_id

    attr_accessor :project_id

    # Subdomain assigned to this deployment (e.g. `my-app-a1b2c3`).
    attr_accessor :subdomain

    # Current deployment lifecycle status.
    attr_accessor :status

    # Fully-qualified URL where the deployment is reachable when `status` is `live`.
    attr_accessor :url

    attr_accessor :created_at

    # Set when the deployment is paused via `/suspend`.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::StudioDeployment` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::StudioDeployment`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [deployment_id, project_id, subdomain, status, url, created_at, suspended_at].hash
    end
