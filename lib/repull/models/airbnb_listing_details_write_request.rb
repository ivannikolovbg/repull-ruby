...
module Repull
  ...
  class AirbnbListingDetailsWriteRequest < ApiModelBase
    ...
    # The specific type inside the group, e.g. `apartment`, `condominium`, `townhouse`, `guesthouse`. Airbnb validates it against the group, so send both when you are changing the kind of property.
    attr_accessor :property_type_category

    # What the guest gets of the property.
    attr_accessor :room_type_category

    # Whole hours on a 24h clock, as strings.
    attr_accessor :quiet_hours
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::AirbnbListingDetailsWriteRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::AirbnbListingDetailsWriteRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'quiet_hours')
        if (value = attributes[:'quiet_hours']).is_a?(Array)
    ...
    def list_invalid_properties
      ...
      if !@quiet_hours.nil? && @quiet_hours.length < 1
    ...
    # @param [Object] quiet_hours Value to be assigned
    def quiet_hours=(quiet_hours)
      if quiet_hours.nil?
        fail ArgumentError, 'quiet_hours cannot be nil'
      end

      if quiet_hours.length < 1
        fail ArgumentError, 'invalid value for "quiet_hours", number of items must be greater than or equal to 1.'
      end
