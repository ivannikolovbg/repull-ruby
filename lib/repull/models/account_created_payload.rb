...
module Repull
  ...
  class AccountCreatedPayload < ApiModelBase
    attr_accessor :workspace_id

    attr_accessor :account_id

    # PMS or channel provider id (e.g. airbnb, booking, hostaway).
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::AccountCreatedPayload` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::AccountCreatedPayload`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [workspace_id, account_id, provider, access_type, created_at].hash
    end
