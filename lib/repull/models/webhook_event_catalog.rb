...
module Repull
  ...
  class WebhookEventCatalog < ApiModelBase
    attr_accessor :domains

    # All events in a flat list (same entries as `domains[].events`, ungrouped).
    attr_accessor :flat
    ...
    def self.openapi_types
      {
        :'domains' => :'Array<WebhookEventCatalogDomainsInner>',
        :'flat' => :'Array<WebhookEventCatalogEntry>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::WebhookEventCatalog` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::WebhookEventCatalog`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flat')
        if (value = attributes[:'flat']).is_a?(Array)
          self.flat = value
        end
      end
    end
