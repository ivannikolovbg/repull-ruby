...
module Repull
  ...
  class GetAirbnbBookingSettings200ResponseDataInstantBook < ApiModelBase
    # `false` when the category is `off` — the listing is request-to-book.
    attr_accessor :enabled

    # Which guests may Instant Book.
    attr_accessor :guest_category

    # `true` for `experienced_guests_only` / `recommended_guests_only` — Airbnb calls this a good track record.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Repull::GetAirbnbBookingSettings200ResponseDataInstantBook` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Repull::GetAirbnbBookingSettings200ResponseDataInstantBook`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
